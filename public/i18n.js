...
const SETTINGS_LABELS_EN = {
  ...
  DESC_CONCURRENCY: 'Description concurrency',
  DESC_HOST_GAP_MS: 'Pause per website (ms)',
  ...
  CREDENTIALS_KEY: 'Encryption key',
  APPLY_DRY_RUN: 'Dry run',
  APPLY_DAILY_CAP: 'Max. applications/day',
  APPLY_COOLDOWN_MINUTES: 'Pause between applications (min.)',
  APPLY_HEADFUL: 'Show browser',
...
const SETTINGS_HELP_EN = {
  ...
  DESC_CONCURRENCY: 'Parallel tabs while loading the job descriptions. Requests are spread across the websites – no matter how high the value, each individual site only ever gets one request at a time.',
  DESC_HOST_GAP_MS: 'Minimum gap between two requests to the same website (plus a random share). This is the value that keeps you off the bot-detection radar – raise it, rather than lowering the concurrency, if a site starts blocking.',
  ...
  AUTO_APPLY_ENABLED: 'Global switch for automated applying (Einfach bewerben / Easy Apply). Off = nothing is ever submitted.',
  CREDENTIALS_KEY: 'Key (64 hex characters) for encrypting the platform credentials. Generate with: npm run generate-credentials-key. Careful: if it changes, every stored credential has to be entered again.',
  APPLY_DRY_RUN: 'On = applications are fully prepared and filled in, but NOT submitted (a screenshot is saved instead of the final click). Leave on until you trust the flow.',
  APPLY_DAILY_CAP: 'Upper limit of submitted applications per client, platform and day (LinkedIn/Indeed are additionally hard-capped at 5).',
  APPLY_COOLDOWN_MINUTES: 'Minimum gap between two submitted applications on the same platform.',
  APPLY_HEADFUL: 'On = the apply browser opens visibly (needed for the first login / 2FA). Only useful on a machine with a display.',
